...
// // Triển khai CANInterfaceWrapper kế thừa ICANInterface
// class CANInterfaceWrapper : public ICANInterface {
// public:
//     CANInterfaceWrapper(const std::string& interface_name) : can_(interface_name) {}
...
//     void publish(const MotorData& data, bool target_reached) {
//         if (std::isnan(data.position) || std::isnan(data.velocity) || std::isnan(data.current)) {
//             RCLCPP_WARN(node_->get_logger(), "Invalid motor data: NaN detected in position, velocity, or current");
//             return;
//         }
//         // if (data.state.empty()) {
//         //     RCLCPP_WARN(node_->get_logger(), "Motor state is empty");
//         // }
...
//         state_msg.data = data.state.empty() ? "UNKNOWN" : data.state;
...
//         current_msg.data = data.current;
//         current_publisher_->publish(current_msg);
//     }

//     void publishResponse(const std::string& message) {
//         if (message.empty()) {
//             RCLCPP_WARN(node_->get_logger(), "Empty response message, skipping publish");
//             return;
//         }
//         std_msgs::msg::String msg;
//         msg.data = message;
...
//           safety_checker_(min_angle_, max_angle_, MAX_MOTOR_TEMPERATURE),
//           state_publisher_(node, name), param_prefix_(name + "."), 
//           last_command_(""), last_command_time_(node_->now()), repeat_(1), repeat_counter_(0) {
//         initializeParameters();
//         setOrigin();
...
//         MotorData updated_data = data;
//         updated_data.state = stateToString(state_);

//         last_data_ = updated_data;
//         // last_data_ = data;

//         if (!safety_checker_.check(updated_data, logger_)) {
//             stop();
//             return;
//         }

//         state_publisher_.publish(updated_data, target_reached_);

//         switch (state_) {
//             case State::INITIALIZING:
//                 if (std::abs(updated_data.position) < POSITION_TOLERANCE) {
//                     is_origin_set_ = true;
//                     transitionTo(State::HOMING);
//                 }
//                 break;
...
//                 if (!last_command_.empty()) {
//                     can_->setPositionSpeed(motor_id_, target_position_, static_cast<int>(speed_), static_cast<int>(accel_));
//                     RCLCPP_INFO(logger_, "Motor 0x%02X cmd not empty!", motor_id_);
//                     if (std::abs(updated_data.position - target_position_) < POSITION_TOLERANCE) {
//                         reach_counter_++;
//                         if (reach_counter_ >= reach_count_max_) {
//                             reach_counter_ = 0;
//                             target_reached_ = true;
...
//                 } else {
//                     if (current_target_index_ >= target_positions_.size()) {
//                         RCLCPP_ERROR(logger_, "Motor 0x%02X: Invalid target index!", motor_id_);
//                         transitionTo(State::STOPPED);
//                         break;
//                     }
//                     float target = target_positions_[current_target_index_];
//                     can_->setPositionSpeed(motor_id_, target, static_cast<int>(speed_), static_cast<int>(accel_));
//                     bool is_reached = std::abs(updated_data.position - target) < POSITION_TOLERANCE;
//                     if (is_reached) {
//                         reach_counter_++;
//                         if (reach_counter_ >= reach_count_max_) {
//                             reach_counter_ = 0;
//                             target_reached_ = true;
//                             current_target_index_++;
//                             RCLCPP_INFO(logger_, "Motor 0x%02X: Reached target %.2f, moving to next",
//                                         motor_id_, target);

//                             if (current_target_index_ >= target_positions_.size()) {
//                                 repeat_counter_++;
//                                 RCLCPP_INFO(logger_, "Motor 0x%02X: Completed cycle %d/%d", 
//                                     motor_id_, current_repeat_, repeat_);
//                                 if (repeat_counter_ >= repeat_) {
//                                     current_target_index_ = target_positions_.size() - 1;
//                                     RCLCPP_INFO(logger_, "Motor 0x%02X: All targets reached %d times.", motor_id_, repeat_);
//                                     stop();
//                                     transitionTo(State::STOPPED);
//                                 } else {
//                                     current_target_index_ = 0; // Đặt lại để lặp lại
//                                     RCLCPP_INFO(logger_, "Motor 0x%02X: Starting repeat %d of %d.", motor_id_, repeat_counter_ + 1, repeat_);
//                                 }
//                             }
//                         }
//                     } else {
...
//             }
//             case State::PAUSED:
//                 can_->setPositionSpeed(motor_id_, target_position_, 
//                                     static_cast<int>(speed_), static_cast<int>(accel_));
                
//                 if (std::abs(updated_data.position - target_position_) < POSITION_TOLERANCE &&
//                     std::abs(updated_data.velocity) < VELOCITY_TOLERANCE) {
                    
//                     if (!target_reached_) { 
//                         target_reached_ = true;
//                         RCLCPP_INFO(logger_, "Motor 0x%02X: Successfully paused at position %.2f", 
//                                     motor_id_, target_position_);
//                     }
//                 } else {
//                     target_reached_ = false;
...
//                 if (can_->setPositionSpeed(motor_id_, target_position_, 
//                                           static_cast<int>(speed_), static_cast<int>(accel_))) {
//                     target_positions_.clear();
//                     target_positions_.push_back(target_position_);
//                     current_target_index_ = 0;
//                     target_reached_ = false; 
//                     last_command_ = "";
//                     response << "Pausing at current position " << target_position_;
//                     RCLCPP_INFO(logger_, "%s", response.str().c_str());
//                     state_publisher_.publishResponse(response.str());
//                     transitionTo(State::PAUSED);
//                 } else {
//                     response << "Failed to pause at position " << target_position_;
//                     RCLCPP_ERROR(logger_, "%s", response.str().c_str());
//                     state_publisher_.publishResponse(response.str());
//                 }
//             }},
...
//     void validateRepeat() {
//         if (repeat_ < 0) {
//             RCLCPP_ERROR(logger_, "Motor 0x%02X: Repeat %d out of bounds [0, inf)", motor_id_, repeat_);
//             throw std::runtime_error("Repeat out of bounds");
//         }
//     }
