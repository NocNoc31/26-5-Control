...
public:
    ...
    //     try {
    //         can_->setCurrent(motor_id_, 0.0);
    //         RCLCPP_ERROR(get_logger(), "Motor 0x%02X: Disabled torque due to system error", 
    //                      controller->motorId());
    //         // Xuất bản lỗi hệ thống
    //         std_msgs::msg::String error_msg;
    //         error_msg.data = "System error: Motor 0x" + std::to_string(controller->motorId()) + " disabled";
    //         system_error_publisher_->publish(error_msg);
    //     } catch (const std::exception& e) {
    //         RCLCPP_ERROR(get_logger(), "Motor 0x%02X: Failed to disable torque: %s", 
...
private:
    ...
    void handleMoving(const MotorData& data) {
        if (mode_ == Mode::MANUAL && !last_command_.empty()) {
            can_->setPositionSpeed(motor_id_, target_position_, static_cast<int>(speed_/2), static_cast<int>(accel_/2));
            if (std::abs(data.position - target_position_) < POSITION_TOLERANCE) {
                reach_counter_++;
                if (reach_counter_ >= reach_count_max_) {
                    reach_counter_ = 0;
    ...
    void handlePaused(const MotorData& data) {
        try {
            // pause_position_ = data.position;
            if (last_command_ == "PAUSE" && 
                (node_->now() - last_command_time_) < rclcpp::Duration::from_seconds(0.5)) {
                // Áp dụng dòng phanh trong 1 giây
                if (can_->setCurrentBrake(motor_id_, 300)) {
                    if (std::abs(data.velocity) < VELOCITY_TOLERANCE) {
                        target_reached_ = true; // Xác nhận động cơ đã dừng
                        RCLCPP_INFO(logger_, "Motor 0x%02X: Brake applied, velocity within tolerance", motor_id_);
                    } else {
                        target_reached_ = false; // Động cơ chưa dừng hoàn toàn
                        RCLCPP_WARN(logger_, "Motor 0x%02X: Still moving during brake, velocity: %.2f", 
                ...
                // Chuyển sang chế độ giữ vị trí
                if (can_->setPositionSpeed(motor_id_, pause_position_, 500, 500)) {
                    target_reached_ = true; // Xác nhận động cơ ở trạng thái giữ vị trí
                    RCLCPP_INFO(logger_, "Motor 0x%02X: Switched to position hold at %.2f, ready for next command", 
                                motor_id_, pause_position_);
                } else {
                    RCLCPP_ERROR(logger_, "Motor 0x%02X: Failed to set position hold", motor_id_);
                    transitionTo(State::ERROR);
                }
            }
        } catch (const std::exception& e) {
    ...
    void handleError(const MotorData& data) {
        try {
            ...
            if (is_braking && (node_->now() - error_start_time) < rclcpp::Duration::from_seconds(0.5)) {
        ...
        command_map_ = {
            ...
            {"PAUSE", [this]() {
                ...
                try {
                    ...
                    if (can_->setCurrentBrake(motor_id_, 300)) {
                        response << "Paused with brake current 100mA at position " << pause_position_;
                        last_command_ = "PAUSE";
                        last_command_time_ = node_->now(); // Ghi thời gian áp dụng phanh
                        target_reached_ = false;
                        RCLCPP_INFO(logger_, "%s", response.str().c_str());
                        state_publisher_.publishResponse(response.str());
                        transitionTo(State::PAUSED);
                    } else {
                        response << "Failed to apply brake with current 100mA";
                        RCLCPP_ERROR(logger_, "%s", response.str().c_str());
                        state_publisher_.publishResponse(response.str());
                        transitionTo(State::ERROR);
                    }
...
private:
    ...
    void stopAllMotors() {
        for (const auto& controller : controllers_) {
            try {
                can_->setCurrent(controller->motorId(), 0.0);
                RCLCPP_ERROR(get_logger(), "Motor 0x%02X: Disabled torque due to system error", 
                             controller->motorId());
